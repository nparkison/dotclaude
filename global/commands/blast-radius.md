...
**Purpose:** Perform standalone blast radius analysis for any code change. Works universally across all projects by reading the universal framework guide and adapting to project-specific mappings when available.
...
1. **Universal Framework (ALWAYS read):**
   - `/mnt/i/My Drive/NP-brain-backup/Claude Code Best Practices/07 - Universal Blast Radius Analysis Framework.md`
...
- Use grep patterns from framework guide to discover dependencies:
...
Use the search patterns documented in the framework guide (section: "How to Trace Dependencies").
...
If this is a Slabstack project OR if project-specific mappings document critical financial queries:
...
Use the risk assessment framework from the guide:
...
Use the **Presentation Template** from the framework guide. Present findings as:
...
- This skill is designed to work in ANY codebase, not just Slabstack
